...
let ChooseNetworkOne = ({ network, state, club_id }) => {
	...
	if (state.contract_address) {
		return (
			<div className='flex justify-between p-4 pl-6 border-b d-border-c-2 h-20 items-center'>
				{t('Manage Deployed Contracts')}
				<Link
					href={
						'/project/' +
						club_id +
						'/manage_contract?network=' +
						network +
						'&address=' +
						state.contract_address
					}
				>
					<a className='btn btn-primary'>{t('manage')}</a>
				</Link>
			</div>
		)
	}
	...
	if (state.contract_address == null && state.is_fetched) {
		return (
			<div className='flex justify-between p-4 pl-6 border-b d-border-c-2 h-20 items-center capitalize'>
				{t('No contract deployed')}
				<Link href={'/project/' + club_id + '/deploy?network=' + network}>
					<a className='btn btn-primary'>{t('deploy')}</a>
				</Link>
			</div>
		)
	}
	...
	return (
		<div className='flex justify-between p-4 pl-6 border-b d-border-c-2 h-20 items-center'>
			{t('Manestudio Contract is not deployed on this network yet')}
		</div>
	)
}
