...
// @withTranslate
@withContractUpdate
class WlMaxSupply extends React.Component {
    ...
    async callContract(value) {
        ...
        await this.props.manenft.request({
            'text' : {
                'loading' : t('update presale max supply'),
                'sent'    : t('update presale max supply tx sent'),
                'success' : t('update presale max supply successful'),
            },
            'func' : {
                'send_tx' : async () => {
                    let tx_in = await this.props.manenft.contract.setPresaleMaxSupply(value);
