...
@withTranslate
@withContractUpdate
class PbLimitPerWallet extends React.Component {
    ...
    async callContract(value) {
        ...
        await this.props.manenft.request({
            'text' : {
                'loading' : t('update public sale limit'),
                'sent'    : t('update public sale limit tx sent'),
                'success' : t('update public sale limit successful'),
            },
            'func' : {
                'send_tx' : async () => {
                    let tx_in = await this.props.manenft.contract.setSaleMaxMintCountPerAddress(value);
...
export default PbLimitPerWallet
