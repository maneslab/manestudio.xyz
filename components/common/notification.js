...
const infoNotification = (config) => {
    config['icon'] = <ExclamationCircleIcon className="text-blue-400 icon-base"/>;
    config['type'] = 'info'
    let cfg = mergeConfig(config);
    notification_open(cfg);
}
...
function loadingNotification(msg) {
    const key = Date.now();
    let icon =  <svg class="animate-spin icon-base text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    notification.notice({
      duration: null,
      key : key,
      content: <div className="block-notification loading">
        <div className="mr-4">{icon}</div>
        <div className="font-ubuntu flex-grow">
            <h2 className="font-bold text-base capitalize title">{msg.message}</h2>
            <p className="text-gray-500 desc">{msg.description}</p>
        </div>
    </div>,
      onClose() {
...
const mergeConfig = (config) => {
    let cfg = {
        ...
        'content'   : <div className={classNames("block-notification",config.type)}>
            <div className="mr-4">{config.icon}</div>
            <div className="font-ubuntu flex-grow">
                <h2 className="font-bold text-base capitalize title">{config.message}</h2>
                <p className="text-gray-500 desc">{config.description}</p>
            </div>
        </div>,
